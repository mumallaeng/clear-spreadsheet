...
- `AXI_Peripheral/`, `ascii-uart-sensor-pipeline/`, `KorchamLogis/`, `wardy/`: 수업 기간과 맞물리지만 구현 소스를 별도 GitHub 저장소로 관리하는 프로젝트로, git submodule로 연결해둡니다.
  ...
  - `wardy`: 9조(지켜조)로 진행한 가정용 돌봄 안전 모니터링 On-Device AI 플랫폼 프로젝트.
